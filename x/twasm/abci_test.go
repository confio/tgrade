	...
	specs := map[string]struct {
		...
		"single callback": {
			setup: func(m *MockSudoer) {
				...
				m.IterateContractCallbacksByTypeFn = iterateContractsFn(t, types.PrivilegeTypeBeginBlock, myAddr)
			},
		...
		"multiple callbacks": {
			setup: func(m *MockSudoer) {
				m.SudoFn = captureSudos(&capturedSudoCalls)
				m.IterateContractCallbacksByTypeFn = iterateContractsFn(t, types.PrivilegeTypeBeginBlock, myAddr, myOtherAddr)
			},
			expSudoCalls: []tuple{
		...
		"no callback": {
			setup: func(m *MockSudoer) {
				m.IterateContractCallbacksByTypeFn = iterateContractsFn(t, types.PrivilegeTypeBeginBlock)
			},
		},
...
func (m MockSudoer) IterateContractCallbacksByType(ctx sdk.Context, callbackType types.PrivilegeType, cb func(prio uint8, contractAddr sdk.AccAddress) bool) {
