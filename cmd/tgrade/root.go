...
func NewRootCmd() (*cobra.Command, app.EncodingConfig) {
	...
	initClientCtx := client.Context{}.
		WithJSONMarshaler(wasmcodec.NewProtoCodec(encodingConfig.Marshaler, encodingConfig.InterfaceRegistry)).
...
func createWasmAppAndExport(
	...
	var emptyWasmOpts []wasm.Option
	if height != -1 {
		tgradeApp = app.NewTgradeApp(logger, db, traceStore, false, map[int64]bool{}, homePath, uint(1), appOpts, emptyWasmOpts)

		if err := tgradeApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		tgradeApp = app.NewTgradeApp(logger, db, traceStore, true, map[int64]bool{}, homePath, uint(1), appOpts, emptyWasmOpts)
	}
